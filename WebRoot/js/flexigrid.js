...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			title: false,
			idProperty: 'id',
			pagestat: '显示 {from} 到 {to}, 共 {total} 项',
			pagetext: '第',
			outof: '页 | 页数:',
			findtext: '查询',
...
//								}
								...
								if(offs >= 0) {
									...
									td.innerHTML = td.innerHTML.replace("[BGCOLOR="+numcolor+"]", "");
								}
			...
			doExport: function(url, type) {
				// 获取url和data 
				var data = 'qtype=' + p.qtype + '&query=' + p.query + '&sortname=' + p.sortname
							+ '&sortorder=' + p.sortorder + '&type=' + type;
				if( url && data ){ 
		...
		if (p.usepager) {
			...
			if (p.searchitems) {
				...
				$(g.sDiv).append("<div class='sDiv2'>" + p.findtext +
						...
						" <select name='qtype'>" + sopt + "</select> <button id='search-clear' class='btn'>清除条件</button></div>");
				...
				$('#search-clear', g.sDiv).click(function () {
	...
	$.fn.flexExport = function(url, type) {
		return this.each(function () {
			if (this.grid && url) this.grid.doExport(url, type);
		});
	};
})(jQuery);
