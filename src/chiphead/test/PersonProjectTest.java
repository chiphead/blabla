...
public class PersonProjectTest {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			int ret = session.insert("PersonProjectMapper.insertPersonProject", perpro);
			session.commit();
			...
			String queryType = "team_name";
	        String queryValue = "杭州";
			...
			projectQueryParm.setFetchNum(12);
			List<ProjectPhase> list = session.selectList("PersonProjectMapper.selectDistinctPersonProjectByParms",projectQueryParm);
			for(int i=0;i<list.size();i++)
				System.out.println(list.get(i));
			int num = session.selectOne("PersonProjectMapper.selectDistinctPersonProjectNumByParms",projectQueryParm);
			System.out.println(num);
			*/
			
			
			List<PersonProject> list = session.selectList("PersonProjectMapper.selectDistinctPersonProjectByTeamOrdered",4);
 			for(int i=0;i<list.size();i++)
 				System.out.println(list.get(i));
			...
			List<String> list = session.selectList("PersonProjectMapper.selectEmpNameByProjNoRoleId",perpro);
 			System.out.println(list);
