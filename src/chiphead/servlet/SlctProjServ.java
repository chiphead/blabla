...
public class SlctProjServ extends HttpServlet {
		...
		try {
	        ...
	        for(int i = 0; i < dList.size(); i++) {
            	...
            	cellMap.put("cell", new Object[] {
            			dProject.getEvaluateState(),dProject.getUpdateTime(),dProject.getReqNo(),
            			dProject.getProjNo(), dProject.getProjName(), dProject.getProjType(), 
            			dProject.getMasterSlave(), dProject.getProjManager(),
	            		dProject.getCurPhase(), dProject.getCurState(), dProject.getRiskQuestion(), dProject.getDesigner(),
	            		dProject.getDeveloper(), dProject.getAuditor(), dProject.getStartDate(), dProject.getActualInstallDate(),
	            		dProject.getPlanEndDate(),dProject.getActualEndDate(), dProject.getActualDays(), 
	            		dProject.getOverdueReason(),dProject.getPlanWorkAmount(), dProject.getActualWorkAmount(),
	            		dProject.getFunctionAmount(), dProject.getProductivity(), dProject.getSmokeNum(), dProject.getStType(),
	            		dProject.getStBugDensity(), dProject.getNc(), dProject.getDesignAudit(), dProject.getCodeAudit(),
	            		dProject.getInstallAudit(), dProject.getExperience(), dProject.getProblemAnalysis(), dProject.getRemark()
	            		});				
	            mapList.add(cellMap);
	        }   
